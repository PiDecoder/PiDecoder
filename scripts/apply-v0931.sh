...
BACKUP="${TARGET}.backup-v0.9.3-${STAMP}"
...
if ! grep -Eq '^[[:space:]]*VERSION[[:space:]]+0\.9\.3\.1([[:space:]]|$)' "$PATCH_DIR/CMakeLists.txt"; then
    echo "ERREUR : CMakeLists.txt ne contient pas VERSION 0.9.3.1" >&2
    exit 1
fi
...
echo
echo "PiDecoder v0.9.3.1 installé."
echo "Duplicate Fix + ajout manuel par IPv4."
echo "Backup : $BACKUP"

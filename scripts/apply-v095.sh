...
BACKUP="${TARGET}.backup-v0.9.4-${STAMP}"
...
if ! grep -Eq '^[[:space:]]*VERSION[[:space:]]+0\.9\.5([[:space:]]|$)' "$PATCH_DIR/CMakeLists.txt"; then
    echo "ERREUR : CMakeLists.txt ne contient pas VERSION 0.9.5" >&2
    exit 1
fi
...
echo
echo "PiDecoder v0.9.5 Advanced Mosaic Layout installé."
echo "Backup : $BACKUP"

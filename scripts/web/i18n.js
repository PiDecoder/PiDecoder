...
(function(global){
  ...
  const STRINGS={
    fr:{
      ...
      'cams.rtsps_enabled':'Connexion chiffrée (RTSPS)',
      'cams.rtsps_enabled_hint':'À cocher uniquement si cette caméra prend en charge le RTSPS (RTSP sur TLS). Le flux vers la caméra est alors chiffré au lieu d\'être en clair sur le réseau. Décoché par défaut : à activer caméra par caméra une fois vérifié que ça fonctionne, le certificat de la caméra n\'étant pas contrôlé.',
      'cams.name':'Nom',
    ...
    en:{
      ...
      'cams.rtsps_enabled':'Encrypted connection (RTSPS)',
      'cams.rtsps_enabled_hint':'Only check this if this camera supports RTSPS (RTSP over TLS). The stream to the camera is then encrypted instead of being sent in cleartext on the network. Off by default: enable it camera by camera once you\'ve confirmed it works, since the camera\'s certificate isn\'t verified.',
      'cams.name':'Name',

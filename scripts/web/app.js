...
let h=d.querySelector('.handle');h.ondragstart=e=>{sync();drag=i;e.dataTransfer.effectAllowed='move';d.style.opacity='.45'};h.ondragend=()=>{drag=null;d.style.opacity='1'};d.ondragover=e=>{if(drag!==null)e.preventDefault()};d.ondrop=e=>{if(drag===null)return;e.preventDefault();let m=cfg.cameras.splice(drag,1)[0];cfg.cameras.splice(i,0,m);drag=null;render()};d.querySelector('.del').onclick=()=>{sync();cfg.cameras.splice(i,1);render();renderMosaic()};d.querySelector('.eye').onclick=()=>{let x=d.querySelector('.pwd');x.type=x.type==='password'?'text':'password'};d.querySelector('.def').onclick=()=>{d.querySelector('.gw').value=640;d.querySelector('.gh').value=360;d.querySelector('.gf').value=12;d.querySelector('.fw').value=1920;d.querySelector('.fh').value=1080;d.querySelector('.ff').value=25};
  // Même retour visuel vert/rouge que pour l'IP manuelle ONVIF et
  // l'adresse manuelle du Réseau (voir setIpv4FieldValidity), pour une
  // présentation homogène de tous les champs d'adresse IP de l'appli.
  const hostInput=d.querySelector('.host');
  setCameraHostValidation(hostInput);
  hostInput.addEventListener('input',()=>setCameraHostValidation(hostInput));
  list.appendChild(d)})}
...
/*
 * Retour visuel homogène (bordure verte/rouge) pour tout champ qui attend
 * une adresse IPv4 ou apparentée — introduit initialement pour l'IP
 * manuelle ONVIF, repris ici pour les mêmes champs dans Réseau (adresse
 * manuelle, passerelle, DNS) et dans la configuration des caméras
 * (adresse IP), pour une présentation cohérente dans toute l'appli. Un
 * champ vide reste neutre (ni vert ni rouge) : on ne juge que ce qui est
 * effectivement saisi.
 */
function setIpv4FieldValidity(input,ok){
  ...
  input.classList.add(ok?'ip-valid':'ip-invalid');
}

function setIpv4Validation(){
  setIpv4FieldValidity(
    manualOnvifIp,
    validIpv4(manualOnvifIp.value.trim())
  );
}
...
function validCidr(value){
  ...
  if(parts.length!==2 || !validIpv4(parts[0]) || !/^\d{1,2}$/.test(parts[1])){
    return false;
  }
...
function validDnsList(value){
  const entries=String(value||'').split(',').map(x=>x.trim()).filter(Boolean);

  return entries.length>0 && entries.every(validIpv4);
}

function setNetworkAddressValidation(){
  setIpv4FieldValidity(networkAddressInput,validCidr(networkAddressInput.value));
}

function setNetworkGatewayValidation(){
  setIpv4FieldValidity(
    networkGatewayInput,
    validIpv4(networkGatewayInput.value.trim())
  );
}

function setNetworkDnsValidation(){
  setIpv4FieldValidity(networkDnsInput,validDnsList(networkDnsInput.value));
}

function setCameraHostValidation(input){
  setIpv4FieldValidity(input,validIpv4(input.value.trim()));
}
...
networkAddressInput.addEventListener('input',setNetworkAddressValidation);
networkGatewayInput.addEventListener('input',setNetworkGatewayValidation);
networkDnsInput.addEventListener('input',setNetworkDnsValidation);
...
function networkLoadConnectionFields(name){
  ...
  setNetworkAddressValidation();
  setNetworkGatewayValidation();
  setNetworkDnsValidation();
}

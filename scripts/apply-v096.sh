...
BACKUP="${TARGET}.backup-v0.9.5.1-${STAMP}"
...
if ! grep -Eq '^[[:space:]]*VERSION[[:space:]]+0\.9\.6([[:space:]]|$)' "$PATCH_DIR/CMakeLists.txt"; then
    echo "ERREUR : CMakeLists.txt ne contient pas VERSION 0.9.6" >&2
    exit 1
fi
...
echo
echo "PiDecoder v0.9.6 Auth & Password Safety installé."
echo "Backup : $BACKUP"

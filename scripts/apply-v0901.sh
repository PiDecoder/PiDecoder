...
BACKUP="${TARGET}.backup-v0.9.0-${STAMP}"
[[ -d "$TARGET" ]] || { echo "Dossier cible introuvable : $TARGET" >&2; exit 1; }
echo "Sauvegarde : $BACKUP"; cp -a "$TARGET" "$BACKUP"
...
if systemctl cat pidecoder.service >/dev/null 2>&1; then systemctl restart pidecoder.service; fi
echo
echo "PiDecoder v0.9.0.1 Discovery Fix installé."
echo "Objectif unique : découverte ONVIF robuste."

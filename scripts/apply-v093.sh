...
BACKUP="${TARGET}.backup-v0.9.2-${STAMP}"
...
if ! grep -Eq '^[[:space:]]*VERSION[[:space:]]+0\.9\.3([[:space:]]|$)' "$PATCH_DIR/CMakeLists.txt"; then
    echo "ERREUR : CMakeLists.txt ne contient pas VERSION 0.9.3" >&2
    exit 1
fi
...
echo
echo "PiDecoder v0.9.3 Camera Manager installé."
